...
<DIV class='title_line'>DELETE MESSAGE</DIV>
    ...
    <UL>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <LI class='li_none'>${param.name } > ${param.title } 카테고리를 삭제했습니다.</LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none'>${param.name } > ${param.title } 카테고리 삭제에 실패했습니다.</LI>
        </c:otherwise>
      </c:choose>
      <LI class='li_none'>
        <br>
