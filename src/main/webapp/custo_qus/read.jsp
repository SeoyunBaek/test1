...
<%--     <c:if test="${custoVO.files.length() > 0 }">
    ...
    <span class='menu_divide' >│</span> 
    <A href='./list_by_custo_search_paging.do?memno=${memVO.memno }&custoqusno=${custoVO.custoqusno }&word=${word}&nowPage=${nowPage}'>목록</A>
    
      <!-- ne => 같지 않을 경우 -->
    <span class='menu_divide' >│</span> 
    <A href='./reply.do?memno=1&custoqusno=1'>답변</A>
    <span class='menu_divide' >│</span> 
...
<%--   <c:if test="${sessionScope.id ne null}"></c:if> --%>
  ...
  <FORM name='frm' method="get" action='./update.do'>
      ...
      <input type='hidden' name='memno' id='memno' value='${custoVO.memno }'>
      <fieldset class="fieldset">
        <ul>
          <li class="li_none">
            <span>${custoVO.title}</span>
            <%-- (<span>${custoVO.name}</span>) --%>
            <span>${custoVO.rdate.substring(0, 16)}</span>
             <DIV>
          ...
          <li class="li_none">
            <DIV>${custoVO.askcont }</DIV>
          </li>
           <li class="li_none">
            <DIV style='text-decoration: underline;'>검색어:(키워드) ${custoVO.word }</DIV>
          </li>
        </ul>
      </fieldset>
  </FORM>
