...
<%--     <c:if test="${custoVO.files.length() > 0 }">
    ...
    <span class='menu_divide' >│</span> 
    <A href='./list_all_employ.do?diecateno=${param.diecateno}'>목록</A>
    
      <!-- ne => 같지 않을 경우 -->
    <span class='menu_divide' >│</span> 
    <A href='./update.do?diecateno=${employVO.diecateno }&employno=${employVO.employno }'>수정</A>
    <!-- &nowPage=${param.nowPage } -->
    <span class='menu_divide' >│</span> 
    <A href='./delete.do?diecateno=${employVO.diecateno }&employno=${employVO.employno }'>삭제</A>
<%--   <c:if test="${sessionScope.id ne null}"></c:if> --%>
 ...
 <input type='hidden' name='diecateno' id='diecateno' value='${diecateVO.diecateno }'>
      <fieldset class="fieldset">
        <ul>
          <li class="li_none">
            <span>${employVO.title}</span>
            <span class='menu_divide' >│</span> 
            <%-- (<span>${custoVO.name}</span>) --%>
            <span>${employVO.rdate.substring(0, 16)}</span>
            <span class='menu_divide' >│</span> 
            <span>${employVO.id}</span>
              <DIV>
          ...
          <li class="li_none">
            <DIV>${employVO.contents }</DIV>
          </li>
<%--            <li class="li_none">
            <DIV style='text-decoration: underline;'>검색어:(키워드) ${employVO.word }</DIV>
          </li> --%>
        </ul>
      </fieldset>
  </FORM>
